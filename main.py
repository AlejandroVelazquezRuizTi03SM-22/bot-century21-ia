...
async def whatsapp_reply(
    ...
    # 0. DETECCIÓN DE AUDIO (WHISPER)
    if NumMedia != "0" and "audio" in MediaContentType0:
        print(f"\n[AUDIO DETECTADO] Descargando nota de voz...")
        texto_transcrito = utils.descargar_y_transcribir_audio(MediaUrl0)
        print(f"[WHISPER ESCUCHÓ] 🗣️: '{texto_transcrito}'")
    ...
    historial = ""
    if cliente_db:
        historial = (cliente_db.get("observaciones_generales") or "")[-600:]
    ...
    def fusionar(campo, es_numero=False):
        ...
        val_db = None
        if cliente_db:
            raw = cliente_db.get(campo)
            val_db = utils.limpiar_numero(raw) if es_numero else utils.limpiar_texto(raw)
    ...
    if datos_finales["origen_campana"]:
        datos_msg["origen"] = datos_finales["origen_campana"]
    ...
    if datos_finales["clave_propiedad"]:
        print(f"[MODO] Búsqueda directa por ID: {datos_finales['clave_propiedad']}")
        ...
        print(f"[ESTADO] Quiere ver: {quiere_ver} | Faltante: {faltante}")

        if faltante == "Ninguno" or quiere_ver:
            usar_mix = (quiere_ver and (not datos_finales["zona_municipio"]))
            propiedades = database.buscar_propiedades(
                datos_finales["tipo_inmueble"],
                datos_finales["zona_municipio"],
                datos_finales["presupuesto"],
                mostrar_mix_general=usar_mix
            )

    # 6. CONSTRUCCIÓN DE LA FICHA TÉCNICA (CON MAYÚSCULAS EXACTAS)
    if propiedades:
        for p in propiedades:
            ...
            link_mapa = p.get('mapa_url') or "No disponible"
        ...
        inventario = "No encontré coincidencias exactas en el sistema C21. Ofreceré opciones similares."
    ...
    try:
        ...
        respuesta = "Dame un momento, estoy consultando el inventario actual de la inmobiliaria."
